...
void snull_release_buffer(struct snull_packet *pkt)
{
	...
	if (netif_queue_stopped(pkt->dev) && pkt->next == NULL)//include in [linux/netdevice.h]
...
int snull_open(struct net_device *dev)
{
	...
	netif_start_queue(dev);//启动传输队列,允许接口接收传输数据包
	return 0;
}
...
static int snull_poll(struct net_device *dev, int *budget)
{
	...
	while (npackets < quota && priv->rx_queue) {
		...
		skb = dev_alloc_skb(pkt->datalen + 2);//申请sk_buff缓冲
		if (! skb) {
			if (printk_ratelimit())
