...
#include <linux/aio.h>	//async io
#include <asm/uaccess.h>	//copy_to(/from)_user
...
int scullc_open (struct inode *inode, struct file *filp)
{
	...
	if ( (filp->f_flags & O_ACCMODE) == O_WRONLY) {//以只写方式打开时
		if (down_interruptible (&dev->sem))//若操作被中断,返回非0值
			return -ERESTARTSYS;
		scullc_trim(dev); /* ignore errors */ //裁剪文件为0
		up (&dev->sem);//释放信号锁
	}

	/* and use filp->private_data to point to the device data */
	filp->private_data = dev;//保存设备特有的信息
