...
{
	int count0;
	DEFINE_WAIT(wait);//建立并初始化一个等待队列项

	while (short_head == short_tail) {//如果为空
		prepare_to_wait(&short_queue, &wait, TASK_INTERRUPTIBLE);//将等待队列项添加到等待队列short_queue中，并设置进程状态
		if (short_head == short_tail)//make sure require to wait.
			schedule();
		finish_wait(&short_queue, &wait);//从等待队列中清除等待队列项wait
...
irqreturn_t short_interrupt(int irq, void *dev_id, struct pt_regs *regs)
{
	...
	/*because "%08u.%06u\n" 's bytes is 16 and unsigned long's max value is more than 10^8,so must use %100 000 000*/
...
//bottom function,be called by top module 
...
//interrupt share 
//irq : IRQ number
//dev_i points to the device 
//regs points to the hardware context
